...
| Инструмент | Описание |
|------------|----------|
| `get_report` | Получить данные HTTP-сервиса конфигурации по относительному пути. Белый список префиксов: `/hs/` и `/odata/standard.odata`; прочие адреса на сервере 1С (`/e1cib/`, служебные точки публикации) отклоняются, чужой хост — тоже (origin сверяется с `ONEC_BASE_URL`). |
...
| `ONEC_WRITE_MODE` | нет | Гейт безопасности записи: `off` (по умолчанию) / `deny` / `preview` / `approval`. См. [Безопасность записи](#безопасность-записи). |
| `ONEC_APPROVAL_TTL_SEC` | нет | Время жизни ожидающего подтверждения, в секундах (по умолчанию `300`). |
| `ONEC_MAX_CONCURRENCY` | нет | Потолок одновременных запросов к 1С на весь процесс (по умолчанию `8`). Один сеанс базы на запрос: на тарифах с 2 сеансами ставьте `1`. |
...
| `deny` | **Только чтение.** 12 пишущих инструментов не регистрируются вовсе — модель их не видит в списке инструментов, а не получает отказ при вызове. В реестре остаются 22 читающих. Штатный режим работ по сверке на чужой базе: на время работ сервер запускается так, что инструменты записи физически отсутствуют. |
...
В режимах `preview` и `approval` появляются два дополнительных инструмента (в `off` и `deny` их нет):
