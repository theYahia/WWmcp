...
export const bp30: ConfigPreset = {
  ...
  pitfalls: [
    ...
    "⚠ Гэп инструментов: виртуальные таблицы регистра БУХГАЛТЕРИИ (Balance / Turnovers / " +
      "BalanceAndTurnovers) ни одним из 32 инструментов не обёрнуты. odata_query их не выразит — " +
      "он URL-кодирует '/' и скобки вызова. get_accumulation_balance работает только с " +
      "AccumulationRegister_*. Пока — читать записи через get_accounting_register и агрегировать " +
      "на клиенте.",
    ...
    "Хозрасчетный — типовое имя регистра; в доработанной базе оно может отличаться. " +
      "Первый шаг на незнакомой базе — list_entities(type=\"all\", search=\"AccountingRegister\").",
  ],
