# MCP-сервер для МойСклад — 10 инструментов для ИИ-агента: остатки, заказы, прибыль

Если вы искали, как подключить МойСклад к Claude или другому ИИ-агенту, — этот сервер даёт агенту доступ к вашему складу: сколько товара осталось и сколько из него в резерве, поиск позиции по артикулу, смена цены, заказ покупателя, приёмка от поставщика, прибыль и маржа по каждому товару за период. Спрашиваете «сколько футболок свободно к продаже» — получаете таблицу с остатками и резервами, а не выгрузку в Excel. Цены во всех инструментах в рублях (перевод в копейки, которых требует API МойСклад, сервер делает сам), лимит запросов соблюдается автоматически.
