...
1. Найди поставщика через `get_counterparties` — возьми meta-href. Если поставщика ещё нет, создай его через `create_counterparty`.
2. Получи организацию через `list_organizations` и склад приёмки через `list_stores` — возьми их meta_href.
...
Если приёмка идёт по заказу поставщику — заказ создаётся через `create_purchase_order`, список заказов смотри через `get_purchase_orders`. Оплату поставщику проведи через `create_payment_out` (банк) или `create_cash_out` (касса).
