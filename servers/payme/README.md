# MCP-сервер для Payme (Узбекистан) — 10 инструментов для ИИ-агента: карты, чеки, приём платежей

Если вы искали, как подключить Payme к Claude или другому ИИ-агенту, — этот сервер даёт агенту приём платежей в Узбекистане целиком: токенизировать карту UzCard или Humo, подтвердить её кодом из SMS, выставить чек, оплатить его или отправить клиенту ссылку на оплату, отменить чек и посмотреть историю за период. Спрашиваете «какие чеки прошли за вчера и на какую сумму» — получаете таблицу, а не выгрузку из кассы. Работает через Payme Subscribe API по JSON-RPC 2.0; по умолчанию сервер стартует в песочнице, боевой контур включается отдельной переменной.
