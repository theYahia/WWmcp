...
1. Call `get_deals` with optional filter_status, filter_responsible_id or search, plus
   limit (max 100) and offset. Page through with offset to the end of the list.
...
7. To chase a deal, call `create_comment` with subject_type = deal, subject_id and text.
...
- Status names are account-specific. If a filter returns empty, pull unfiltered and group
