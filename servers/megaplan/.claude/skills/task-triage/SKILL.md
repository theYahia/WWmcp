...
2. Call `get_tasks` with filter_status (e.g. active, delayed), optionally
   filter_responsible_id, limit (max 100) and offset. Page through with offset until a
   short page comes back — do not report a total you have not paged to.
...
6. To unblock or chase, call `create_comment` with subject_type = task, subject_id and text
   (HTML supported). Confirm the wording with the user before posting — comments notify people.
...
- filter_status values depend on the account's configuration. If a filter returns nothing,
  fetch unfiltered and group client-side rather than guessing a status name.
