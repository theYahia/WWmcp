...
export function createServer(): McpServer {
  ...
  server.tool(
    ...
    "Push money TO a customer's Orange Money wallet (B2B disbursement). Requires the customer's MSISDN (phone number).",
    {
      ...
      amount: z.string().regex(/^\d+(\.\d{1,3})?$/, "Amount as a plain positive decimal, e.g. '500' or '500.00'").describe("Amount as string in local currency"),
  ...
  server.tool(
    ...
    "Pull money FROM a customer's Orange Money wallet (B2B collection). Customer must have pre-authorized the merchant.",
    {
      ...
      amount: z.string().regex(/^\d+(\.\d{1,3})?$/, "Amount as a plain positive decimal, e.g. '500' or '500.00'").describe("Amount as string"),
