# MCP-сервер для ЦБ РФ — курсы валют, ключевая ставка и драгметаллы (7 инструментов)

Даёт ассистенту официальные данные Центробанка России: курс любой валюты на нужную дату и его динамику за период, текущую ключевую ставку и историю её изменений, учётные цены на золото, серебро, платину и палладий, а также пересчёт суммы из валюты в валюту по курсу ЦБ. Ключей и регистрации не требует — источники публичные.
...
| `get_rate_dynamics` | Динамика курса валюты за период: ряд значений и сводка — min/max/avg и изменение за период |
| `get_key_rate` | Текущая ключевая ставка ЦБ РФ в процентах и дата, с которой она действует. Параметров не принимает |
| `get_key_rate_history` | История изменений ключевой ставки за период: даты вступления в силу и значения. По умолчанию — последний год |
...
- «Покажи, как ЦБ менял ключевую ставку с начала года».
- «Насколько вырос доллар по курсу ЦБ за август?»
...
| `get_rate_dynamics` | `cbr.ru/scripts/XML_dynamic.asp` |
| `get_key_rate`, `get_key_rate_history` | `cbr.ru/hd_base/KeyRate` — официальная таблица ставки |
