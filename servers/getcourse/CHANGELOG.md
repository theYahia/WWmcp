...
Все значимые изменения `@theyahia/getcourse-mcp` документируются здесь.
...
- 3 инструмента: `get_users`, `create_user`, `get_deals`. Ключ API передаётся
  query-параметром.
...
- Первый релиз: пользователи и сделки GetCourse.
