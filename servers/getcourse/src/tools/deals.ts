...
export const getDealsSchema = z.object({
  ...
  ]).optional().describe("Filter by deal status"),
  created_from: z.string().optional().describe("Created date from (YYYY-MM-DD)"),
  created_to: z.string().optional().describe("Created date to (YYYY-MM-DD)"),
});
...
export async function handleGetDeals(params: z.infer<typeof getDealsSchema>): Promise<string> {
  ...
  const exportResp = (await gcGet("/account/deals", {
  ...
  if (!exportResp.success && exportResp.info?.export_id) {
    ...
    return "Deal export is taking too long. Try again later.";
  }
