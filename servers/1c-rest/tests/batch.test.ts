...
describe("batch tools", () => {
  ...
  it("batch_update_catalog_items: PATCHes each item by Ref_Key", async () => {
    ...
    const result = await handleBatchUpdateCatalogItems({
      ...
      updates: [
        { ref_key: "aaa-111", data: { Description: "A" } },
        { ref_key: "bbb-222", data: { Description: "B" } },
      ],
      concurrency: 5,
    });
    ...
    expect(parsed.succeeded).toBe(2);
    expect(parsed.failed).toBe(0);
    expect(urls.some((u) => u.includes("aaa-111"))).toBe(true);
    expect(urls.some((u) => u.includes("bbb-222"))).toBe(true);
  });
