...
- Tool count: 9 → 24. Bumped server version to 3.1.0.
- Tests: 27 → 57 (added `tests/oauth-refresh.test.ts` 10 cases + `tests/webhook-signature.test.ts` 17 cases + catalog-op + coverage-tool cases in `tests/tools.test.ts`).
